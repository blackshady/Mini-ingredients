...
define container_err
	${INFO} "Please execute \"make start\" on a different terminal"
endef
...
start:
	@ docker volume create --name=Mini_ingredients_data > /dev/null
	@ echo "  "
...
clean:
	...
	@ docker-compose -f $(DOCKER_DEV_COMPOSE_FILE) down -v
	@ docker volume rm Mini_ingredients_data
